...
but only 156 users: about 161 resets per user. Interpreted in plan section **D9** — that week is
`avni-client#2115`, a reset that re-arms itself, so it is evidence of a defect's cost rather than a
workload to model.
